    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        protected virtual void LoadMatchingAssemblies(string directoryPath)
        {
            ...
            foreach (var dllPath in Directory.GetFiles(directoryPath, "*.dll"))
            {
                ...
                {
                    Trace.TraceError(fileNotFoundException.ToString());
                }
            }
        }
        ...
        public IEnumerable<Type> ClassesWithAttribute<TAttribute>(bool onlyConcreteClasses = true) where TAttribute : Attribute
        {
            var result = new List<Type>();
            var assemblies = GetAssemblies();
            foreach (var a in assemblies)
            {
                var types = a.ClassesWithAttribute<TAttribute>();
                foreach (var t in types)
                {
                    if (onlyConcreteClasses)
                    {
