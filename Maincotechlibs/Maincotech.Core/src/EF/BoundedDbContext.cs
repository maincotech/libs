...
{
    public abstract class BoundedDbContext : DbContext, IDbContext
    {
        #region Constructor

        public BoundedDbContext(DbContextOptions options) : base(options)
        {
        }
        ...
        protected abstract IEnumerable<IMappingConfiguration> GetConfigurations();
        ...
        {
            var tables = this.GetCurrentTables();
            if(tables.Any() == false)
            {
